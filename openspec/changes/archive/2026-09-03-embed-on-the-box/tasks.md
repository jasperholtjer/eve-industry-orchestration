...
- [x] 1.1 In `defs/news.py`, change `_EMBED_POOL` (line ~265) from
...
- [x] 1.2 Make the matching edit in `defs/transcripts.py` for
...
- [x] 1.3 In `defs/market_orders.py`, correct the comment at line ~76-80 that
...
- [x] 1.4 Real run: with `CORPUS_BINARY_PATH` pointed at
...
- [x] 2.1 Change `news_daily_schedule`'s `cron_schedule` from `"0 22 * * *"`
...
- [x] 2.2 Real run: preview one tick of `news_daily_schedule` and one tick of
...
- [x] 3.1 In `deploy/dagster.yaml`: remove the `news_embed` pool from the
...
- [x] 3.2 In `deploy/redeploy.sh`: both existing
...
- [x] 3.3 Real run: reproduce `redeploy.sh`'s own `validate_instance_config`
...
- [x] 4.1 In `tests/test_concurrency_pools.py`, drop `"news_embed"` from
...
- [x] 4.4 In `tests/test_context_datasets.py`, the two assertions at ~162-165
...
- [x] 4.5 `CONTEXT.md` (~line 60, "Four exist: ...") and `README.md` (~line
...
- [x] 4.2 Write `docs/adr/0002-heavy-pool-membership-for-exclusion-not-peak.md`
...
- [x] 5.1 `uv run ruff check . && uv run ruff format --check . && uv run
