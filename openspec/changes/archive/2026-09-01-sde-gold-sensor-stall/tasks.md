...
- [x] 1.1 Add an optional `sort_key` parameter to
...
- [x] 1.2 Update the `request_partitions` docstring and the module docstring so
...
- [x] 2.1 Consult the `dagster-expert` skill before touching the sensor
  ...
  `SensorResult` API.
- [x] 2.2 Teach `tests/fake_corpus.py` to answer a `state query` for committed
...
- [x] 3.1 Rewrite `sde_gold_sensor` to read both run-state queries, drop builds
...
- [x] 3.2 Rewrite the sensor's docstring to state the readiness rule it now
...
- [x] 4.1 Rewrite the two existing Gold-sensor tests in `tests/test_sde.py`,
...
- [x] 4.2 Add a regression test for the stall itself: with more outstanding
...
- [x] 4.3 Add tests for the remaining spec scenarios — a build with committed
