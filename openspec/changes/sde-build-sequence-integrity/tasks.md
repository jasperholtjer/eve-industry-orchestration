...
- [ ] 1.1 Add a read-only `stale_changelog_builds()` to
...
- [ ] 1.2 Teach `tests/fake_corpus.py` to answer that query. It dispatches on
...
- [ ] 2.1 Consult the `dagster-expert` skill before touching either sensor
...
- [ ] 2.2 Rewrite `sde_build_discovery_sensor` in
...
- [ ] 2.3 Carry `release_date` as a label rather than discarding it: name the
...
- [ ] 3.1 Fold the stale term into `sde_gold_sensor`'s outstanding set: it becomes
...
- [ ] 3.2 Defer, in the same sensor, any outstanding build that has a *lower*
  ...
  with `uv run pytest -q tests/test_sde.py`.
- [ ] 3.3 Update the sensor's docstring to state all three terms and the deferral,
...
- [ ] 4.3 Deferral: an outstanding build with a lower `sde_silver` run in flight is
  ...
  `uv run pytest -q tests/test_sde.py`.
- [ ] 4.4 Confirm the existing Gold-sensor tests from `sde-gold-sensor-stall` still
  pass unchanged, or say in the report which one changed and why. Verify with
  `uv run pytest -q tests/test_sde.py`.

## 5. The record

- [ ] 5.1 Write `docs/adr/0001-sde-changelog-gold-is-rebuildable.md`, this
