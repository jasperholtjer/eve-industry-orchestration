...
- [ ] 1.5a Pin `sov-panel`'s zero reach-back directly on the lookback, not through a resolved start date: `_silver_start` takes the minimum preload, so a wrong panel value of 30 is discarded by the `min` and moves no date. Verify with `uv run pytest tests/test_config.py -q`.
