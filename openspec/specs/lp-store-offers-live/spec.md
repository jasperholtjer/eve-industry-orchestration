# lp-store-offers-live Specification

## Purpose

Defines how the two live LP-store Gold trees are refreshed and recorded: one
asset for one ESI fan-out rather than one asset per tree, the daily cadence and
why availability is not sensed, the absence of any concurrency pool, the
per-derivative row counts the materialisation carries out of a status object
that is multi-partition, and the run-state read and verify call the code
location must not attempt.
