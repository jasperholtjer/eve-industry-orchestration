...
derivative, ``sovereignty-contests`` (``shape: sov-contests``, no look-back and
no coverage gate), so ``resolve_partition_starts`` needs no derivative selector;
that build lands in a later row.
